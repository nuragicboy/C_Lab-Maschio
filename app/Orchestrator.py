...
class Orchestrator:
    ...
    def run(self):
        ...
        for step in self.profile["steps"]:
            ...
            elif (step["action"] == "readCSVCharLimit"):
                try:
                    self.excelData = pd.readCSVCharLimit(self.dataPath, columns=step["columns"], skiprows=step["SkipRows"])
                except:
                    err="Errore nella lettura del file\n\n"+traceback.format_exc()
                    return[-1,err]
            ...
            elif (step["action"] == "takeFirstPerGroup"):
                try:
                    self.excelData = pd.takeFirstPerGroup(self.excelData, step["column"])
                except:
                    err="Errore durante il fill dei campi vuoti\n\n"+traceback.format_exc()
                    return[-1,err]
            ...
            pd.writeLocal(self.excelData, self.dataPath + str(test) + str(step["action"]) + ".xlsx", "Test/",header=True)
        ...
        pd.writeLocal(self.excelData, self.dataPath + "REN" + ".xlsx", "Test/", header=True)
        ...
        pd.writeLocal(self.excelData, self.dataPath + "DEF" + ".xlsx", "Test/", header=True)
        ...
        for step in self.profile["analysis"]:
            if (step["action"] == "checkRange"):
                ...
                try:
                    s=self.checkRange(step)
                except:
                    err="Errore durante l'update dei dati tramite db\n\n"+traceback.format_exc()
                    print(err)
                    return[-1,err]

        pd.writeLocal(self.excelData, "def.xlsx", "Test/",header=True)
    ...
    def removeExtraColumns(self):
        query="select campo_interno from dizionario where lab in ('"+self.profileName+"','all') order by campo_interno"
        results=self.dab.select(query)
    ...
    def checkRange(self,step):
        ...
        results=self.dab.select(query)
        columnNames= {str(row[0]) for row in results}
        res = pd.between(self.excelData, results)
        return res
